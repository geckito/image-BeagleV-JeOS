...
zypper ar -f -n "home:Andreas_Schwab:riscv:beaglev (standard)" http://download.opensuse.org/repositories/home:/Andreas_Schwab:/riscv:/beaglev/standard/ home_Andreas_Schwab_riscv_beaglev
...
if [[ $kiwi_iname == *-beaglevstarlight ]]; then
    cat > /etc/dracut.conf.d/beaglevstarlight_modules.conf <<EOF
add_drivers+=" dw_mmc-pltfm "
EOF
    cat > /etc/modprobe.d/50-beaglevstarlight.conf <<EOF
